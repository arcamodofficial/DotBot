...
namespace KittyMemory {
    ...
    void *MemCpy(void *dest, const void *src, size_t len) {
        // ARCAMOD: Optimized memory copy for better performance
        if (!dest || !src || len == 0) return dest;

        // Use optimized copy for different sizes
        if (len >= 64) {
            // For large copies, use system memcpy which is highly optimized
            return memcpy(dest, src, len);
        } else if (len >= 8) {
            // For medium copies, use 64-bit chunks
            ...
            // For small copies, use byte-by-byte (original method)
    ...
    {
        // ARCAMOD: Optimized validation - early exit for better performance
        if (!address || !buffer || !len) {
            #ifdef DEBUG
            if (!address) KITTY_LOGE("memWrite err address (%p) is null", address);
            if (!buffer) KITTY_LOGE("memWrite err buffer (%p) is null", buffer);
            if (!len) KITTY_LOGE("memWrite err invalid len");
            #endif
            return false;
        }
