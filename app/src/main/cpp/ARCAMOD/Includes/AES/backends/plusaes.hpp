...
// Detect if exceptions are disabled
#if defined(__cpp_exceptions) || defined(__EXCEPTIONS) || defined(_CPPUNWIND)
    #define PLUSAES_USE_EXCEPTIONS 1
    #include <stdexcept>
#else
    #define PLUSAES_USE_EXCEPTIONS 0
    // No include stdexcept when exceptions are disabled
#endif
...
/**
 * @private
 * @return round count or 0 if key_size is invalid
 */
inline unsigned int get_round_count(const int key_size) {
    ...
    case 32:
        return 14;
    default:
        #if PLUSAES_USE_EXCEPTIONS
        throw std::invalid_argument("Invalid key size");
        #else
        return 0; // Return invalid count instead of throwing exception
        #endif
    }
}
...
/**
 * @private
 * @return Round keys or empty vector if key_size is invalid
 */
inline RoundKeys expand_key(const unsigned char *key, const int key_size) {
    if (key_size != 16 && key_size != 24 && key_size != 32) {
        ...
        throw std::invalid_argument("Invalid key size");
        #else
        return RoundKeys(); // Return empty roundkeys
        #endif
    }
